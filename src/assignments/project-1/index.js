...
class Uploads extends React.Component {
    ...
    render() {
        ...
        return <div id="grid-container">
            <button id='faq-toggle-btn' onClick={this.toggleFaq.bind(this)}>{buttonText}</button>
            {!this.state.faqActive && <h1 id="gallery-header">Gallery</h1>}
            {!this.state.faqActive && <ul class="album">
                {completedFiles.map(file => {
                    return <Album file={file}/>
                })}
            </ul> }

            {!this.state.faqActive && <div class="uploader-container">
